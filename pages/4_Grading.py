...
from core.ebay import (
    ebay_is_configured, get_access_token, fetch_orders, normalize_orders_to_rows,
    upsert_ebay_order_rows, apply_matched_orders_to_inventory, fetch_order_earnings_summary, sync_listings,
)

st.set_page_config(page_title="eBay Store", layout="wide")
st.title("eBay Store")

st.caption("This page syncs eBay orders into Google Sheets. Inventory is updated only when the eBay line item SKU equals your inventory_id.")
...
if not ebay_is_configured():
    st.warning("eBay is not configured yet. Add ebay_client_id, ebay_client_secret, and ebay_refresh_token to Streamlit secrets.")
    with st.expander("Required secrets", expanded=True):
        st.code('''
...
''')
    ...
    with col3:
        update_inventory = st.checkbox("Mark matched inventory as SOLD after sync", value=True)
    ...
    if st.button("Sync orders", type="primary"):
        try:
            ...
            inv_ids = set(inv["inventory_id"].astype(str).str.strip()) if not inv.empty else set()
            rows = normalize_orders_to_rows(orders, inv_ids)
            inserted = upsert_ebay_order_rows(rows)
            changed = apply_matched_orders_to_inventory(rows, inv) if update_inventory else 0
            st.success(f"Fetched {len(orders):,} order(s), added {inserted:,} new line item row(s), updated {changed:,} inventory row(s).")
            refresh_database_cache()
            st.rerun()
        except Exception as exc:
            st.error(str(exc))
    ...
    st.caption("This uses eBay Finances order earnings summary when your developer app has access to that scope.")
...
if orders_df.empty:
    ...
    m2.metric("eBay gross", money_fmt(orders_df["sold_price"].sum()))
    m3.metric("eBay net", money_fmt(orders_df["net_proceeds"].sum()))
    unmatched = orders_df[~orders_df["matched_to_inventory"].astype(str).str.upper().isin(["YES", "TRUE", "1"])]
...
if orders_df.empty:
    ...
    view = orders_df.sort_values("sold_date", ascending=False)
    cols = ["sold_date", "ebay_order_id", "ebay_line_item_id", "sku", "inventory_id", "title", "sold_price", "shipping_charged", "fees_total", "net_proceeds", "matched_to_inventory", "sync_status"]
...
if listings_df.empty:
    ...
    cols = ["sku", "inventory_id", "title", "listing_status", "price", "quantity", "offer_id", "listing_id", "last_synced_at"]
    st.dataframe(listings_df[[c for c in cols if c in listings_df.columns]], use_container_width=True, hide_index=True)
